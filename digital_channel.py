# FILE: digital_channel_simple.py
# Phiên bản KHÔNG có mã hóa/giải mã kênh LDPC, tương tự bài báo VQ-VAE.
...
# --- Bảng tra cứu điều chế/giải điều chế 16-QAM ---
QAM16_CONSTELLATION = {
...
def modulate_16qam(bits: np.ndarray) -> np.ndarray:
    ...
    # Pad bit 0 nếu độ dài không chia hết cho 4
...
def simulate_rayleigh_channel(symbols: np.ndarray, snr_db: float) -> np.ndarray:
    ...
    # Chuẩn hóa công suất tín hiệu để tính toán nhiễu chính xác hơn
    signal_power = np.mean(np.abs(symbols)**2)
    if signal_power == 0: return symbols # Tránh chia cho 0
    symbols_normalized = symbols / np.sqrt(signal_power)

    # Tính toán công suất nhiễu
    ...
    # Cân bằng kênh (giả định biết chính xác h)
    equalized_signal = received_signal / (h + 1e-8)
    
    # Khôi phục lại công suất ban đầu
...
def transmit_and_receive_indices_batch(indices_tensor: torch.Tensor, vq_bits_map: torch.Tensor, snr_db: float):
    ...
    for i in range(batch_size): # Lặp qua từng ảnh trong batch
        # 1. Bit Packing
